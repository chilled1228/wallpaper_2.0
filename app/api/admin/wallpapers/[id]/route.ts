...
const WallpaperSchema = z.object({
  ...
  additionalImages: z.array(ImageMetadataSchema).max(5, 'Maximum 5 additional images allowed').optional().or(z.array(z.any())),
...
export async function GET(
  ...
  try {
    ...
    const wallpaperRef = db.collection('wallpapers').doc(params.id);
...
export async function PUT(
  ...
  try {
    ...
    console.log(`PUT /${params.id}: Received wallpaper data:`, rawData);
    ...
    try {
      ...
      let newSlug = params.id;
      if (currentData?.title !== title) {
        newSlug = providedSlug || await generateUniqueSlug(title, db);
        console.log(`Generated new slug: ${newSlug} (old: ${params.id})`);
        ...
        if (newSlug !== params.id) {
          ...
          console.log('Creating new document with data:', updateData);
          await newWallpaperRef.set(updateData);
          
          // Delete old document
          console.log(`Deleting old document with ID: ${params.id}`);
      ...
      console.log(`Updating document ${params.id} with data:`, updateData);
      await wallpaperRef.update(updateData);
